...
class MaJoDo {
  ...
  start(httpDataLimit: string = "50mb"): void {
    const app = new MajodoHttpServer(
      this.serverAddress,
      this.httpServerPort,
      httpDataLimit
    );
  ...
  private initGameServer(): void {
    if (this.type === ServerType.UDP) {
      const udpServer = new UDPGameServer(
        this.serverAddress,
        this.udpServerPort
      );
      ...
      const wsServer = new WebSocketGameServer(
        this.httpServer,
        this.type === ServerType.WS_WITH_PROTOBUFS
      );
      ...
      throw new Error(
        "Unsupported server type. Choose 'UDP', 'WS_WITH_PROTOBUFS' or 'WS'."
      );
    }
  }
    ...
    setInterval(() => {
      ...
      MaJoDo.lastReceivedUserTimestamps.forEach((lastReceived, uid) => {
        if (
          ...
          MaJoDo.roomsToClient[roomName] = MaJoDo.roomsToClient[
            roomName
